    ...
    <div style={{
      ...
      <svg width={W} height={H_SVG} style={{ overflow: 'visible', flexShrink: 0 }}>
        ...
        <line
          ...
          stroke="var(--color-accent-dim)" strokeWidth="1.8" strokeOpacity="0.9"
        />
        {/* Δx / Δy right-angle helper */}
        {Math.abs(h) > 0.08 && (() => {
        ...
        {/* Curve */}
        <polyline points={curvePts.join(' ')} fill="none" stroke="var(--color-text)" strokeWidth="2" />
        {/* Point A */}
        ...
        {/* h bracket */}
        <line x1={ax} y1={ay + 24} x2={bx} y2={ay + 24} stroke="var(--color-border)" strokeWidth="1" />
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '24px' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--color-text-muted)' }}>
            A = <span style={{ color: 'var(--color-accent)' }}>({a}, {fa.toFixed(3)})</span>
          </div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--color-text-muted)' }}>
            B = <span style={{ color: 'var(--color-accent-dim)' }}>({(a + h).toPrecision(3)}, {fb.toFixed(3)})</span>
          </div>
        </div>
        <div style={{ borderTop: '1px solid var(--color-bg-subtle)', paddingTop: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--color-text-muted)' }}>
            할선 기울기 = Δy / Δx
          </span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '15px', color: 'var(--color-accent)', fontWeight: 'bold' }}>
            {slope.toFixed(5)}
          </span>
        </div>
